@extends('layouts.app') @section('title','Scoreboard') @section('content') {{-- add a jumbotron here --}}

<div class="container" id="myContainer">
    
	<div class="row" id="myJumbo">
            <div class="col-md-6">
            	<h3>{{$batting1st}} : {{$total_runs1st}}/{{$wickets1st}}</h3>
            	@foreach ( $current_batsmen1st as $batsman)
                <span> 
        ...
        <div class="col-md-8">
            ...
            <div class="tab-content" id="myTabContent">
                <div id="1st" role="tabpanel" class="tab-pane fade in active">
                    ...
                    @else
                    <h2><strong style="color:red">The Match is not start yet . Wait untill it starts.</strong></h2> 
                    @endif {{-- end 1st column --}} {{-- start column for 2nd innigs --}}
                </div>
        ...
        <div class="col-md-4">
            ...
            <div class="tab-content">
                <div class="tabpane fade in active" role="tabpanel" id="1stcmntry">
                	@if($start1st>0)
                    {{-- whrite commentry code here --}} @foreach($commentaries1st as $commentary)
                    <ul class="list-group">
                    ...
                    @endforeach {{$commentaries1st->render()}}
                    @else
                    <h2><strong style="color:red">The Match is not start yet. Wait untill it starts.</strong></h2>
                    @endif
                </div>
                <div class="tabpane fade" role="tabpanel" id="2ndcmntry">
                	@if($start2nd>0)
                    {{-- whrite commentry code here --}} 
                    @foreach($commentaries2nd as $commentary)
                    <ul class="list-group">
                        <li class="list-group-item">
                            <button class="btn btn-danger btn-circle">{{$commentary->over}}</button>
                            {{$commentary->commentary}}, <strong style="color: red; font-size: 20px;">{{$commentary->run}}</strong>
